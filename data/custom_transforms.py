...
class RandomGaussianBlur(object):
    def __call__(self, sample):
        if random.random() < 0.5:
            sample['image'] = sample['image'].filter(ImageFilter.GaussianBlur(
...
class FixScaleCrop(object):
    ...
    def __call__(self, sample):
        ...
        if w > h:
            ...
            oh = int(1.0 * h * ow / w)
        img = img.resize((ow, oh), Image.BILINEAR)
        seg = seg.resize((ow, oh), Image.BILINEAR)
        mask = mask.resize((ow, oh), Image.NEAREST)
        # center crop
        w, h = img.size
        x1 = int(round((w - self.crop_size) / 2.))
        y1 = int(round((h - self.crop_size) / 2.))
        sample['image'] = img.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
        sample['seg'] = seg.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
        sample['label'] = mask.crop((x1, y1, x1 + self.crop_size, y1 + self.crop_size))
...
class MultiScale(object):
    ...
    def __call__(self, sample):
        ...
        return sample
class MultiScale_Val(object):#new add

    def __init__(self, size, resize_list):
        self.resize_list = resize_list
        self.size = size
    def __call__(self, sample):
        h = self.size[0]
        w = self.size[1]
        resize_image = [F.resize(sample['image'], (int(h/i), int(w/i)), interpolation=Image.BILINEAR) for i in self.resize_list]
        sample.update({'image_multiscale':resize_image})
        return sample
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for key in single_targets:
            if key == 'image_multiscale':#new add 
                sample[key] = [F.to_tensor(item) for item in sample[key]]
                continue
